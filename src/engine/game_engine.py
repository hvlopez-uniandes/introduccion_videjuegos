...
class GameEngine:
    def __init__(self, cfg_dir=None):
        # Raíz del proyecto (subo dos carpetas desde engine/)
        self._root = Path(__file__).resolve().parents[2]
        if cfg_dir is None:
            self._cfg_dir = self._root / "src" / "cfg" / "cfg_00"
        else:
    ...
    def _create(self):
        ...
        esper.add_component(player_entity, CColor(player_cfg["r"], player_cfg["g"], player_cfg["b"]))
